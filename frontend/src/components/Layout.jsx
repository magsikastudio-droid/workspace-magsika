    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileOpen(true)} className="rounded-full border border-slate-200 bg-white p-2.5 text-slate-700 hover:bg-slate-50 lg:hidden">
              <Menu size={20} />
            </button>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 overflow-y-auto rounded-r-3xl bg-white p-5 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500">Hi,</p>
                <p className="text-lg font-semibold">{user?.name || "Admin User"}</p>
              </div>
              <button onClick={() => setMobileOpen(false)} className="rounded-full bg-slate-100 p-2 text-slate-700 hover:bg-slate-200">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="mt-6 border-t border-slate-100 pt-4">
              <button onClick={logout} className="flex w-full items-center justify-between rounded-2xl px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">
                Logout <LogOut size={16} />
              </button>
            </div>
          </div>
