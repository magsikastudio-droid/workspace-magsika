...
export default function Layout({ children }) {
      ...
      <div className="px-4 py-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 dark:border-white/[0.06] bg-slate-50 dark:bg-white/5 py-2 pl-9 pr-3 text-xs text-slate-700 dark:text-slate-300 outline-none transition focus:border-violet-300 dark:focus:border-violet-700 focus:bg-white dark:focus:bg-white/10 placeholder:text-slate-400 dark:placeholder:text-slate-600"
          />
        </div>
      </div>
    ...
    : 0
      ...
      {role === "admin" && (
        <div className="border-t border-slate-100 dark:border-white/[0.06] px-4 py-3">
          <div className="flex items-center justify-between rounded-full bg-slate-50 dark:bg-white/5 px-3.5 py-2">
            ...
            <button
              ...
              className="rounded-full bg-white dark:bg-white/10 px-3 py-1 text-xs font-bold text-slate-700 dark:text-slate-200 shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-100 dark:hover:bg-white/20"
            >
              {currency}
            </button>
          </div>
      ...
      <div className="border-t border-slate-100 dark:border-white/[0.06] px-4 py-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={logout} title="Logout" className="rounded-full p-1.5 text-slate-400 transition hover:bg-slate-100 dark:hover:bg-white/10 hover:text-rose-500">
            <LogOut size={15} />
          </button>
        </div>
      </div>
                ...
                <div className="flex gap-2">
                  {FEELINGS_LOCK.map((f) => (
                    <button key={f.value} type="button" onClick={() => setLockForm((p) => ({ ...p, feelings: f.value }))}
                      className={`flex items-center gap-1.5 rounded-full border-2 px-4 py-2 text-sm font-semibold transition hover:-translate-y-0.5 ${lockForm.feelings === f.value ? f.active : "border-slate-200 bg-white text-slate-500 hover:border-slate-300"}`}>
                      {f.emoji} {f.value}
                    </button>
                  ))}
                </div>
  ...
  return (
    <div className="fixed inset-0 flex gap-3 overflow-hidden bg-slate-100 p-3 dark:bg-[#0a0a0a]">
      <BirthdayBanner />
      {alarmBannerVisible && (
        <OverdueAlarmBanner
          tasks={overdueAlarms}
      ...
      <aside className="hidden w-56 shrink-0 overflow-hidden rounded-[28px] border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#141414] shadow-sm lg:flex lg:flex-col">
        <SidebarContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-56 overflow-hidden rounded-r-[28px] bg-white dark:bg-[#141414] shadow-2xl">
            <div className="absolute right-3 top-3 z-10">
              <button onClick={() => setMobileOpen(false)} className="rounded-full p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 transition">
                <X size={16} />
              </button>
            </div>
            <SidebarContent />
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-hidden">
        <header className="relative z-10 flex items-center justify-between rounded-[24px] border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#141414] px-5 py-3.5 shadow-sm">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileOpen(true)} className="rounded-full p-1.5 text-slate-500 dark:text-slate-400 transition hover:bg-slate-100 dark:hover:bg-white/10 lg:hidden">
              <Menu size={20} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-2">
            {(role === "admin" || role === "pm") && (
              <button
                ...
                title="Notifikasi"
                className="relative rounded-full border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/5 p-2 text-slate-500 dark:text-slate-400 transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-500 dark:hover:border-indigo-700 dark:hover:bg-indigo-900/20"
              >
                <Bell size={16} />
            ...
            <button onClick={logout} title="Logout" className="rounded-full border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/5 p-2 text-slate-500 dark:text-slate-400 transition hover:-translate-y-0.5 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-500 dark:hover:border-rose-800 dark:hover:bg-rose-900/20">
              <LogOut size={16} />
            </button>
          </div>
        </header>
...
function NavItem({ item, badge = 0 }) {
  if (item.external) {
    return (
      <a
        ...
        className="flex items-center gap-2.5 rounded-full px-3.5 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-slate-200"
      >
  ...
  return (
    <NavLink
      to={item.to}
      className={({ isActive }) =>
        `flex items-center gap-2.5 rounded-full px-3.5 py-2 text-sm transition-all duration-200 ${
          isActive
            ? "bg-violet-50 dark:bg-violet-500/10 font-semibold text-violet-700 dark:text-violet-400 shadow-sm"
            : "font-medium text-slate-600 dark:text-slate-400 hover:-translate-y-0.5 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-slate-200"
        }`
      }
    >
      {({ isActive }) => (
